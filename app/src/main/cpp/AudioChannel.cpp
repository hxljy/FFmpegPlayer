        ...
        : BaseChannel(stream_index, pContext, time_base)
{
    ...
    out_buffers = static_cast<uint8_t *>(malloc(out_buffers_size)); // 在堆中
...
/**
 * 音频解码（运行在子线程）
 * 从压缩队列中 取出 压缩数据  ---》（解码）后的原始数据包 放入到  原始队列中
 */
void AudioChannel::audio_decode() {
    AVPacket * packet = 0; // AVPacket 专门 存放 压缩数据包 音频AAC
    while (isPlaying) {
        // 生产快  消费慢
        ...
        ret = avcodec_send_packet(pContext, packet); // 把餐具给食堂阿姨，（食堂阿姨内部维持一个队列）
        if (ret) {
        ...
        ret = avcodec_receive_frame(pContext, frame); // 食堂阿姨，把打好的饭菜给--> AVFrame(原始数据)
        if (ret == AVERROR(EAGAIN)) { // 音频 也有帧的概念
            // 重来，重新取
            continue;
        } else if(ret != 0) {
        ...
        // 终于取到了，解码后的音频数据（原始数据） == PCM

        frames.push(frame); // （原始数据队列）加入队列
    }
    // 出了循环，就要释放
    releaseAVPacket(&packet);
}
...
void AudioChannel::audio_player() {
    ...
    SLresult result = slCreateEngine(&engineObject, 0, NULL, 0, NULL, NULL); // 执行完成后，engineObject 有值了
    ...
    result = (*engineObject) ->GetInterface(engineObject, SL_IID_ENGINE, &engineInterface); // 执行完成后，engineInterface 有值了
    if (SL_RESULT_SUCCESS != result) {swr_convert
        return;
    }
